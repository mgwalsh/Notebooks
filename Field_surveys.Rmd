...
This provides a plot of the ROI, and extracts the needed spatial coordinates of the sampling frame. Note that these constraints are intended to keep the ROI within a generally accessible range for field surveyors (by car, motorcycle or on foot). The ROI can then be plotted as a simple raster object:
...
You can also use similar protocols for geotagging any rock, soil, plant and livestock biomarker samples at the locations that you visit. All physical samples should receive *Universally Unique IDs* that are embedded in [QR-code labels](https://tag.qed.ai/sheet/) for each sample, which are tagged with metadata and the relevant O&M in the field. This process also allows tracking any sample through various (MIR, XRF and/or wet chemistry) laboratory processes and to curate physical samples at your various laboratory sample storage facilities.
