...
set.seed(6405)                      ## sets randomization seed, note if you change this you will get different results
B <- cbind(p, rmask[,1], rmask[,2]) ## specifies spatial balancing variables
rsamp <- cube(p, B)                 ## samples from the ROI
```

The next chunk generates geenerates various output files, which can be used to for survey planning and field navigation.
