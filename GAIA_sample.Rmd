...
The table below is the main output data frame `samp` of the proposed trial locations with their lon/lat coordinates, which can be sorted by Level 1 & 2 administrative units and/or `GID`. The `GID` is like a military style 10x10 km grid ID that may contain several sampling locations depending on the spatial distribution of potential sites (see `N`, above) within the ROI. Our practice has been to prioritize field activities in `GID` that contain at least several trial locations. The data frame is also written as a csv file `GAIA_sample.csv` into your working directory, which you can use as waypoint input to a GPS (see e.g: [GPSBabel](https://www.gpsbabel.org)), tablet or smart phone for in-the-field navigation.
