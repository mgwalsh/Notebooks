...
The table below is the main output data frame `samp` of the proposed trial locations with their lon/lat coordinates, which can be sorted by various administrative units and/or `GID`. The `GID` is sort of like a military style 10x10 km grid ID that will usually contain several sampling locations. Our practice has been to prioritize field activities in `GID` that contain at least several trial locations. The data frame is also written as a csv file `GAIA_sample.csv` into your working directory, which you can use as waypoint input to a GPS (see e.g: [GPSBabel](https://www.gpsbabel.org)), tablet or smart phone for in-the-field navigation.
